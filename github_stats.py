...
class Queries(object):
    """
    用于查询 GitHub GraphQL (v4) API 和 REST (v3) API 的类
    """
    ...
    async def query(self, generated_query: str) -> Dict:
        """
        向 GraphQL API 发送请求
        :param generated_query: 要发送的查询字符串
...
"""
    ...
    def all_contribs(cls, years: List[str]) -> str:
        """
        :param years: 要获取贡献的年份列表
        :return: 获取所有年份贡献信息的查询
        """
...
async def main() -> None:
    ...
    if access_token is None or user is None:
        raise RuntimeError(
            "必须设置 ACCESS_TOKEN 和 GITHUB_ACTOR 环境变量!"
        )
